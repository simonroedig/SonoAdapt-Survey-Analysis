...
\textbf{Omnibus Main Effect Tests:} Before examining specific pairwise differences, we first verified whether each factor had a statistically significant overall (omnibus) effect on each dependent variable using Type III Wald $\chi^2$ tests. Only factors with a significant omnibus effect were subsequently interpreted via pairwise comparisons. As shown in \autoref{tab:omnibus_tests}, nearly all main effects reached significance. The single exception was the effect of Notification Type on Detectability ($\chi^2(2) = 7.47, p = .024$), which did not survive the Bonferroni-corrected threshold. Consequently, pairwise comparisons for this specific factor--dimension combination are not reported.

\textbf{Pairwise Comparisons:} For factors with a significant omnibus effect, we extracted all pairwise comparisons by systematically re-fitting the LMM with shifted reference levels for each factor. \autoref{tab:lmm_pairwise} details these comparisons. The resulting values ($\Delta M$) represent the absolute point-difference on our 7-point Likert scale between two specific factor levels. 
...
% ==========================================
% OMNIBUS MAIN EFFECT TESTS TABLE
% ==========================================
\begin{table}[H]
\centering
\caption{Omnibus main effect tests (Type III Wald $\chi^2$) for each factor across the three dependent variables. All factors were tested with $df = 2$. Significance threshold: Bonferroni-corrected $\alpha = .0167$.}
\label{tab:omnibus_tests}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Factor} & \textbf{Disruption} & \textbf{Social Acc.} & \textbf{Detectability} \\
\midrule
Notification Type & $\chi^2 = 191.91$*** & $\chi^2 = 61.89$*** & $\chi^2 = 7.47$\hphantom{***} \\
Social Setting    & $\chi^2 = 12.18$**\hphantom{*}  & $\chi^2 = 218.02$*** & $\chi^2 = 10.43$**\hphantom{*} \\
Task Load         & $\chi^2 = 202.71$*** & $\chi^2 = 58.08$***  & $\chi^2 = 29.03$*** \\
Soundscape        & $\chi^2 = 39.97$***  & $\chi^2 = 76.38$***  & $\chi^2 = 109.81$*** \\
\bottomrule
\multicolumn{4}{l}{\small \textit{Note:} * $p < .0167$, ** $p < .01$, *** $p < .001$. All tests with $df = 2$.}
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Linear Mixed Model (LMM) pairwise comparison results for factors with a significant omnibus main effect (see \autoref{tab:omnibus_tests}). Values ($\Delta M$) indicate the absolute point-difference on the 7-point Likert scale between the two compared levels. \textcolor{blue}{Blue} indicates a significant positive shift for the first level compared to the second, \textcolor{red}{Red} indicates a significant negative shift. The significance threshold is Bonferroni-corrected to $p < .0167$. \textbf{Framed cells} highlight the primary relationships targeted by our study design. \textcolor{gray}{Gray} cells indicate comparisons where the omnibus main effect was not significant; pairwise values are shown for completeness but are not interpreted.}
...
Short Speech vs. Earcon & \textcolor{blue}{1.14***} & \textcolor{red}{-0.63***} & \textcolor{gray}{0.18\hphantom{***}} \\
Rich Speech vs. Earcon & \textcolor{blue}{1.57***} & \textcolor{red}{-0.86***} & \textcolor{gray}{0.26\hphantom{***}} \\
Rich Speech vs. Short Speech & \textcolor{blue}{0.43***} & -0.23\hphantom{***} & \textcolor{gray}{0.08\hphantom{***}} \\
\midrule
...
Notably, the omnibus main effect of notification type on detectability did not reach statistical significance after Bonferroni correction ($\chi^2(2) = 7.47, p = .024$; threshold: $p < .0167$). While a descriptive trend suggests that speech-based formats (Rich Speech: $M = 6.01$; Short Speech: $M = 5.92$) may be slightly easier to detect than Earcons ($M = 5.74$), this difference cannot be confirmed statistically and pairwise comparisons for this factor are therefore not interpreted.

Analyzing the descriptive interaction between soundscape and notification type reveals distinct auditory masking patterns. In a completely quiet environment, all notification types are highly detectable (Earcon: $M = 6.21$; Rich Speech: $M = 6.59$). However, in an environment with background music, the Earcon becomes noticeably harder to detect ($M = 5.33$), while spoken notifications cut through the music more easily (Rich Speech: $M = 6.14$). Conversely, when the ambient soundscape consists of background speech, this effect reverses: the Earcon remains moderately detectable ($M = 5.69$), but the detectability of Short Speech ($M = 5.23$) and Rich Speech ($M = 5.31$) drops. These descriptive patterns suggest that background speech directly masks incoming speech notifications, whereas music primarily masks abstract tonal alerts.
