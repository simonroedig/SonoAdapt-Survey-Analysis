...
To protect against Type I errors (false positives) that arise from running multiple parallel tests, we applied a Bonferroni correction for the post-hoc pairwise comparisons across our three main models (Disruption, Social Acceptability, Detectability). The threshold for pairwise statistical significance was thus strictly adjusted to $\alpha = .0167$ ($.05 / 3$). 

\textbf{Omnibus Main Effect Tests:} Before examining specific pairwise differences, we first verified whether each factor had a statistically significant overall (omnibus) effect on each dependent variable using Type III Wald $\chi^2$ tests at the standard $\alpha = .05$ level. Only factors with a significant omnibus effect were subsequently interpreted via pairwise comparisons. As shown in \autoref{tab:omnibus_tests}, all main effects reached significance across all dependent variables, confirming that task load, social setting, soundscape, and notification type each meaningfully influence user perception.

\textbf{Pairwise Comparisons:} For each significant omnibus effect, we extracted all pairwise comparisons by systematically re-fitting the LMM with shifted reference levels for each factor. \autoref{tab:lmm_pairwise} details these comparisons. The resulting values ($\Delta M$) represent the absolute point-difference on our 7-point Likert scale between two specific factor levels. 
...
\begin{table}[H]
\centering
\caption{Omnibus main effect tests (Type III Wald $\chi^2$) for each factor across the three dependent variables. All factors were tested with $df = 2$. Significance threshold: $\alpha = .05$.}
...
\bottomrule
\multicolumn{4}{l}{\small \textit{Note:} * $p < .05$, ** $p < .01$, *** $p < .001$. All tests with $df = 2$.}
\end{tabular}
\end{table}
...
We also found a significant omnibus main effect of notification type on detectability ($\chi^2(2) = 7.47, p = .024$). Specifically, \textit{Rich Speech} ($M = 6.01$) was significantly easier to detect overall than an abstract Earcon ($M = 5.74$; $\Delta M = 0.26, p = .007$). \autoref{fig:box_detectability}b details these findings across all notification formats.
